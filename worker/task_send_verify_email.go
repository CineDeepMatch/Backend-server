...
func (proccessor *RedisTaskProcessor) ProcessTaskSendVerifyEmail(ctx context.Context, task *asynq.Task) error {
	...
	log.Info().Msg("proccessed task")

	verifyEmail, err := proccessor.store.CreateVerifyEmail(ctx, db.CreateVerifyEmailParams{
		UserID:     user.ID,
		Email:      user.Email,
	...
	proccessor.mailer.SendEmail(subject, content, to, nil, nil, nil)
